...
dygraph(chxrnsa, main = "Home Price Index for Chicago, Illinois") %>%
  ...
  dySeries("pred.chsht", label = "Prediction for High Tier", strokeWidth = 4, strokePattern = c(7, 3)) %>%
  dySeries("pred.chsmt", label = "Prediction for Middle Tier", strokeWidth = 4, strokePattern = c(7, 3)) %>%
  dySeries("pred.chslt", label = "Prediction for Low Tier", strokeWidth = 4, strokePattern = c(7, 3)) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(3, "Dark2")) %>%
  dyShading(from = "2018-1-1", to = "2018-12-1") %>%
  dyLegend(width = 600)
